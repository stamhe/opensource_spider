...
class opensource_python_spider(CrawlSpider):
    # 爬虫的识别名称，必须是唯一的，在不同的爬虫中你必须定义不同的名字
    name = "python_spider"    # 设置爬虫名称

    # 搜索的域名范围，也就是爬虫的约束区域，规定爬虫只爬取这个域名下的网页
    # https://www.python.org/ftp/python/
    allowed_domains = ["www.python.org"] # 设置允许的域名
    ...
    def parse(self, response):
        ...
        pt1 = re.compile(r'<a\shref="(.*?)">(.*?)</a>([\s]*)([-\w\d]*)\s([:\d]*)([\s]*)([-\d\w]*)([\s]*)')
