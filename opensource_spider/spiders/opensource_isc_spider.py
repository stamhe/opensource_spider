...
class opensource_isc_spider(CrawlSpider):
    # 爬虫的识别名称，必须是唯一的，在不同的爬虫中你必须定义不同的名字
    name = "isc_spider"    # 设置爬虫名称

    # 搜索的域名范围，也就是爬虫的约束区域，规定爬虫只爬取这个域名下的网页
    # https://ftp.isc.org/isc/
    allowed_domains = ["ftp.isc.org"] # 设置允许的域名
    ...
    def parse(self, response):
        ...
        pt1 = re.compile(r'<img\ssrc="(.*)"\salt="\[(.*)\]"\s/></td><td\sclass="indexcolname"><a\shref="(.*)">(.*)</a>')
