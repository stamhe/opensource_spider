...
class opensource_curl_spider(CrawlSpider):
    # 爬虫的识别名称，必须是唯一的，在不同的爬虫中你必须定义不同的名字
    name = "curl_spider"    # 设置爬虫名称

    # 搜索的域名范围，也就是爬虫的约束区域，规定爬虫只爬取这个域名下的网页
    # https://curl.haxx.se/download/
    allowed_domains = ["curl.haxx.se"] # 设置允许的域名
    ...
    def parse(self, response):
        ...
        pt1 = re.compile(r'<img\ssrc="(.*?)"\salt="\[(.*?)\]"></td><td><a\shref="(.*)?">(.*?)</a></td>')
        ret1 = re.findall(pt1, response.text)
        for v in ret1:
            if v[1] == 'DIR':
                new_url = response.url + v[2]
